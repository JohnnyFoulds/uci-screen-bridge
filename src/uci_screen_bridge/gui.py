...
def main():
    ...
    def log_process(process, finish_message):
        ...
        while True:
            ...
            if process.poll() is not None:
                ...
                break
        start_btn = tk.Button(button_frame, text="Start Game", command=start_game)
        start_btn.grid(row=0, column=0)
        board_btn = tk.Button(button_frame, text="Board Calibration", command=board_calibration)
        board_btn.grid(row=0, column=1)
    ...
    def diagnostic(ignore=None):
        nonlocal running_process
        arguments = [sys.executable, "-m", "uci_screen_bridge.diagnostic"]
    ...
    save_file = data_path('gui.bin')

    def save_promotion(*args):
        outfile = open(data_path("promotion.bin"), 'wb')
